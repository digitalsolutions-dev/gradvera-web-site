...
**not** replace `astro check` — see `tests/e2e/README.md` for how to run it
locally and add checks. `tests/` is excluded from `tsconfig` so neither harness
feeds the `astro check` gate.

## Layout

- `src/pages/` — routes. Astro i18n with `prefixDefaultLocale: false`: EN at `/`,
  SL at `/sl/`, HR at `/hr/`. Each locale has `index`, `book-a-demo/`,
  `privacy-policy/`, `construction-cost-estimation/`,
  `construction-bid-estimate/` (SL/HR use localized slugs — see
  `src/i18n/slugs.ts`). Plus the **EN-only** acquisition landing page
  `construction-estimating-software/` (indexed; hreflang `en` + `x-default`
  only; sitemap 0.9; the language switcher sends SL/HR to their home) —
  EN-only routes are registered in `EN_ONLY_ROUTES` (`src/i18n/slugs.ts`) and
  honoured by `alternates()`, the sitemap and `LangSwitch`.
- `src/pages/api/lead.ts` — lead-capture endpoint (see below).
- `src/lib/` — pure, unit-tested helpers (`leadScore.ts`, `leadPayload.ts`).
...
- `src/i18n/` — `en.json` / `sl.json` / `hr.json`, plus `_parts/` fragments (EN
  ...
  landing-page copy — no SL/HR keys). `utils.ts` = translation helpers.
...
- `src/styles/` — design-origin CSS, imported in cascade order by
  ...
  `src/scripts/site.js` + `attribution.js` — interactions, bundled the same way.
